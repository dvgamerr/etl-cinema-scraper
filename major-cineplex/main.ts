...
export async function SearchMovieNowShowing(page: Page): Promise<CinemaItem[]> {
  ...
  const eMovieCard = await page.waitForFunction(
    `document.querySelectorAll('div#movie-page-showing div.ml-box')`,
  );
  const cinemaItems: CinemaItem[] = await page.evaluate(
    scrapingCinema,
    eMovieCard,
  );
...
export async function SearchMovieCommingSoon(
  page: Page,
): Promise<CinemaItem[]> {
  const eMovieCard = await page.waitForFunction(
    `document.querySelectorAll('div#movie-page-coming div.ml-box')`,
  );
  const cinemaItems: CinemaItem[] = await page.evaluate(
    scrapingCinema,
    eMovieCard,
...
export function scrapingCinema(elements): CinemaItem[] {
  ...
  for (const e of elements) {
    ...
    const [name] =
      link.replace('/movie/', '').replace(/\W+/ig, '-').match(/[\w-]+$/) || [];
